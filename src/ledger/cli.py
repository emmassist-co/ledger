...
from ledger.config import load_config, load_dotenv_file
from ledger.archive_index.dr_build import build_dr_outer_map, write_dr_outer_map
...
from ledger.explorer import write_explorer_html
from ledger.extract.page_map import build_page_map
from ledger.io.paths import DocumentPaths
from ledger.io.readers import read_episode_note
...
from ledger.models.artifacts import (
    ArtifactLink,
    ClaimArtifact,
    EpisodeNote,
    GenerationEvent,
...
from ledger.pipeline.stages.parse_pdf import parse_pdf
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    episode_generator = _build_generator(
        ...
        fake_output=os.getenv("LEDGER_FAKE_LLM_OUTPUT"),
        stage="episode_note",
    )
...
def _build_generator(*, model: str, fake_output: str | None, stage: str):
    ...
    if not api_key:
        raise RuntimeError(
            "Set LEDGER_OPENROUTER_API_KEY or OPENROUTER_API_KEY, or set LEDGER_FAKE_LLM_OUTPUT for local testing."
        )
...
def _render_index_file(view: ViewArtifact) -> str:
    ...
    from ledger.render.frontmatter import render_frontmatter
