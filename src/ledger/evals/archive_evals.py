...
def normalize_question_shape_policies(payload: object) -> dict[str, dict]:
    if isinstance(payload, dict):
        return {
            str(name): policy
            for name, policy in payload.items()
...
def _decision_record_exceeds_query_budget(archive_root: Path, result: dict) -> bool:
    ...
    bounded_search = question_shape_policies.get(question_shape, {}).get("bounded_search", {})
